...
//		closeFrame('standBillMenu');
	...
	else if(type=="TAKEPHOTO"){//图片上传
		var standBillModel = $api.getStorage("standBillModel");
		console.log("1:"+standBillModel.billSn+",2:"+standBillModel.status+",3:"+standBillModel.id)
		var opts ={"billSn":standBillModel.billSn, "orderlink":standBillModel.status, "billid":standBillModel.id};
		api.openFrame({
	...
	else if(type=="AUDIT"){//审核（需审核页面开发人员修改传参
		var standBillModel = $api.getStorage("standBillModel");
		var pageParam = standBillModel;
	...
	else if(type=="WORKCENTER"){//施工中图片上传
		var standBillModel = $api.getStorage("standBillModel");
		console.log("1:"+standBillModel.billSn+",2:"+standBillModel.status+",3:"+standBillModel.id)
		var opts ={"billSn":standBillModel.billSn, "orderlink":standBillModel.status, "billid":standBillModel.id};
		api.openFrame({
	...
	else if(type=="RETURNKEY"){//离站确认图片上传
		api.openWin({
			name : 'stationKeyReturn_win',
			url : '../newOutStation/stationKeyReturn_win.html',
			bounces : false,
			rect : {
			 x : 0,
			 y : 0,
...
//	}
	 ...
	 }else if(type=="FACECHECK"){
		openBillList("billStationFaceCheck_win");
//		api.closeFrame();
...
//	远程开门zhangning
	...
	function getDoorsLists() {
		var firstDate = new Date();
			...
			console.log("传入远程开门的数据2---》"+JSON.stringify(opts));
			api.showProgress({
				title : '处理中',
				modal : true,
			});
			$client.getStationOpenDoor(opts, function(ret, err) {
			console.log("远程开门的数据返回结果---》"+JSON.stringify(ret));
			api.hideProgress();
			var optsDoorStatus="2";//开门失败
			var msg="";//出错消息
			//远程开门
			if (ret) {
				if(ret.success==true){
			...
			optsDoor.operatingCost = (endDate.getTime()-firstDate.getTime())/1000; 
			console.log("============远程开门返回数据保存传参==============="+JSON.stringify(optsDoor));
...
//		document.getElementById('acceptSnId').innerHTML = standBillModel.applyid;
		...
		if(api.pageParam.fromTab=='standBillApplyingListFrm'){
			tpl = $api.byId('standBillMenu-template2').text;
		}
